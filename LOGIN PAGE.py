...
def Database():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS `member` (mem_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username TEXT, password TEXT, firstname TEXT, lastname TEXT, dob TEXT)")
USERNAME = StringVar()
PASSWORD = StringVar()
FIRSTNAME = StringVar()
LASTNAME = StringVar()
...
def RegisterForm():
    ...
    lbl_dob = Label(RegisterFrame, text="dob:", font=('arial', 18), bd=18)
    lbl_dob.grid(row=5)
    ...
    dob = Entry(RegisterFrame, font=('arial', 20), textvariable=DOB, width=15)
...
def Login():
    ...
    if USERNAME.get == "" or PASSWORD.get() == "":
        ...
        if cursor.fetchone() is not None:
            lbl_result1.config(text="Login Successfull", fg="blue")
            webbrowser.open("https://www.github.com/sudhakar1798")
